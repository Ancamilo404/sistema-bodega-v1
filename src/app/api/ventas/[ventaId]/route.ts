...
// PUT /api/ventas/:id → actualizar campos parciales
export async function PUT(req: Request, { params }: { params: { ventaId: string } }) {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // Manejo de stock según transición de estado
...
// PATCH /api/ventas/:id → confirmar venta
export async function PATCH(req: Request, { params }: { params: { ventaId: string } }) {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!venta) throw new Error("Venta no encontrada");
      if (venta.estado === "CONFIRMADA") throw new Error("La venta ya está confirmada");
...
// DELETE /api/ventas/:id → alias de anulación
